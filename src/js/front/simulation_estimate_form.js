if ($('#simulation_register_form').length) { 
  ...
  var sendForm = function (el) {
    ...
    $('#simple_simulation_estimate_btn p').text('送信中...');
   ...
   * Check Contact
   */
  $('#simple_simulation_estimate_btn').on('click', function() {
  ...
  $('input[name=lpgas_contact\\[estimate_kind\\]]').on('click', function() {
    if (form_hidden) {
      ...
      $('.simple-simulation-estimate-btn').removeClass('btn-disabled');
      $('.simple-simulation-estimate-btn i').removeClass('fa-angle-up');
      $('.simple-simulation-estimate-btn i').addClass('fa-angle-right');
      $('.simple-simulation-estimate-btn p').text('【無料】WEBで料金プランを見る');
      form_hidden = false;
    }
  ...
  $('.simple-simulation-estimate-btn').on('click', function() {
