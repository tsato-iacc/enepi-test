...
<%= link_to "現地調査表(pdf)出力", url_for(format: :pdf), class: "btn btn-default", target: "_blank" %>
...
<% if @estimate.verbal_ok? || @estimate.contracted? %>
  <%= form_for [:partner, @estimate], url: partner_lpgas_estimate_update_misc_path(@estimate) do |f| %>
    <table class="table table-striped table-hover">
      <tr>
        <th>STEP1. 連絡する</th>
        <td>
