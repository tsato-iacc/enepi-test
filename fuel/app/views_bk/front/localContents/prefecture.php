<% title "【#{@prefecture_itself.name}】プロパンガス(LPガス)料金の適正価格と相場！" %>
<% description "#{@prefecture_itself.name}のプロパンガス料金を知りたい方はこちらをチェック！お住まいの地域ごとに平均的なガス代を調べられます。プロパンガス(LPガス)は地域によって料金が異なるので、平均的なガス代を把握し、見直しに役立ててください。" %>
<div class="article-maintitle-area">
  <div class="inner">
    ...
    <div class="text">
      <h1 class="title" itemprop="headline"><%= @prefecture_itself.name %>×プロパンガス(LPガス)の平均利用額はココでチェック!</h1>
      <ul class="categories">
        <li><i class="icon icon-tag"></i><%= link_to "LPガス/プロパンガス", "/categories/lpgas" %></li>
      </ul>
      <p class="description" itemprop="description">
        エネピ完全オリジナル！<%= @prefecture_itself.name %>のプロパンガス料金を調べることができます。石油情報センターより提供される金額を掛け合わせて算出しております。<br>
        ガス代が高いと感じている方は、このページで<%= @prefecture_itself.name %>の適正料金を調べて安いガス会社へ切り替えましょう！
      </p>
    </div>
  </div>
</div>
        ...
        <div class="local-content-middle">
          ...
          <div class="local-content-preface">
            <p class="local-content-middle-intro">
              <%= @prefecture_itself.name %>でプロパンガスをご利用になられている方はこちらのページをご覧ください。<br><% if @prefecture.number_of_approved_company != 0 %> ゴールド保安認定事業者(第一号認定LPガス販売事業者)に認定されている<%= @prefecture_itself.name %>のプロパンガス会社は<%= @prefecture.number_of_approved_company %>社います。<br><% end %>金額の見直しを検討される方はプロパンガス会社の無料お見積もり依頼をお試しください!<br>ページ下のボタンをクリックし、フォームに必要な情報を入力して頂くと、エネピに加盟している厳選ガス会社をご紹介致します。
            </p>
            <span>(2017年6月度更新のデータを参照)</span>
          </div>
          <div class="average-rate-box">
            <div class="average-rate-box-title">
               <h3><%= @prefecture_itself.name %>のプロパンガス平均価格</h3>
            </div>
            <div class="average-rate-box-content">
              <div class="average-rate-box-basic">
                <dl>
                  <dt class="average-rate-box-data-title">基本料金</dt>
                  <% if @prefecture.basic_rate != 0 %>
                    <dd class="average-rate-box-data-detail"><%= @prefecture.basic_rate.to_s(:delimited) %>円</dd>
                  <% else %>
              ...
              <div class="average-rate-box-basic">
                <dl>
                  ...
                  <% if @prefecture.commodity_charge_criterion != 0 %>
          ...
          <div class="average-usage-box">
            <h3 style=" margin: 0;"><%= @prefecture_itself.name %>の人たちが使っている<br>プロパンガス(LPガス)の平均使用量</h3>
            <div class="average-usage-box-circle">
              <div class="average-usage-box-data">
                <% if @prefecture.annual_average != 0 %>
                  <%= @prefecture.annual_average.round(1) %>㎥
                <% else %>
                  -
                <% end %>
              </div>
          ...
          <div class="average-rate-list-box">
            <h3 class="average-monthly-rate-title"><%= @prefecture_itself.name %>の使用量別<br>1ヶ月の平均プロパンガス料金</h3>
            <table summary="<%= @prefecture_itself.name %>の使用量別1ヶ月の平均プロパンガス料金" border width="80%" height="200" cellspacing="0" cellpadding="0" style="margin: auto;">
              <tr align="center">
              ...
              <tr align="center">
                ...
                <% if @prefecture.five_cubic_meters_price != 0 %>
              ...
              <tr align="center">
                ...
                <% if @prefecture.ten_cubic_meters_price != 0 %>
              ...
              <tr align="center">
                ...
                <% if @prefecture.twenty_cubic_meters_price != 0 %>
              ...
              <tr align="center">
                ...
                <% if @prefecture.fifty_cubic_meters_price != 0 %>
          ...
          <div class="city-list-box">
            <h3 class="title-price-info"><%= @prefecture_itself.name %>の各市区町村</h3>
            <span>お住まいの市区町村をクリックすると、市区町村別の料金ページが見れます</span>
            <div class="city-list">
              <% if @regions.present? %>
                <ul　style="padding-left: 0;">
                  <% @regions.each do |region| %>
                    <li class="city-list-content"><%= link_to "・#{region.city_name}","/local_contents/city_show/#{region.id}" %></li>
                  <% end %>
                </ul>
              <% end %>
            </div>
          ...
          <% if @reviews.present? %>
            ...
            <div class="word-of-mouth-box">
              <div class="word-of-mouth-title"><h3 class="word-of-mouth-title-itself"><%= @prefecture_itself.name %>の口コミ一覧</h3></div>
                    ...
                    <li class="word-of-mouth-list">
                      <div class="word-of-mouth-header">
                        <div class="reviewer-info">
                          ...
                          <span class="personal-info"><%= @prefecture_itself.name %>
                          <% if r.city_code != 0 %>
                            <%= Region.find(r.city_code).city_name %>
                          <% end %>
                          在住/</span>
