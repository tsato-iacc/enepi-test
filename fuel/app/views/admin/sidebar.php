...
<div class="list-group sidebar">
  ...
  <a href="<?= \Uri::create('admin/estimates') ?>" class="list-group-item list-group-item-action<?= \Request::active()->controller == 'Controller_Admin_Estimates' && in_array(\Request::active()->action, ['index', 'show']) ? ' active' : ''; ?>"><i class="fa fa-table"></i>見積もり依頼一覧</a>
  <a href="<?= \Uri::create('admin/activity') ?>" class="list-group-item list-group-item-action<?= \Uri::segment(2) == 'activity' ? ' active' : ''; ?>"><i class="fa fa-table"></i>対応リスト</a>
