<?= simple_form_for [:admin, @company] { |f| ?>
  ...
  <h3>ガス会社情報</h3>
  <?= f.simple_fields_for :lpgas_company, @company.lpgas_company || @company.build_lpgas_company { |f| ?>
    ...
    <div id="company_service_features">
      ...
      <?= f.fields_for :company_service_features { |uf| ?>
        <?= uf.hidden_field :id ?>
    ...
    <div class="form-group">
      <? if f.object.persisted? ?>
        <? if f.object.lpgas_company_image.try!(:url) ?>
          <?= MyView::image_tag(f.object.lpgas_company_image.url, style: "max-height: 200px; border: 1px solid #CCC;" ?>
        <? } ?>
      <?end ?>
    </div>
    ...
    <div class="form-group">
      <? if f.object.persisted? ?>
        <? if f.object.lpgas_company_logo.try!(:url) ?>
          <?= MyView::image_tag(f.object.lpgas_company_logo.url, style: "max-height: 200px; border: 1px solid #CCC;" ?>
        <? } ?>
      <?end ?>
    </div>
