...
<% if @contact.original_contact %>
  <p class="alert-paragraph">
    <%= link_to "問い合わせID=#{@contact.original_contact.id} (#{@contact.enum_value_i18n(:status)})", admin_lpgas_contact_path(@contact.original_contact) %> の再入力CVです。
  </p>
<% end %>
...
<% if smart_phone? %>
  <table class="table table-condensed table-striped table-hover">
    ...
    <tr>
      <th>自動見積もり</th>
      <td><% if @contact.sent_auto_estimate_req %>◯<% else %>×<% end %></td>
    </tr>
    <tr>
      <th>提示画面閲覧済み</th>
  ...
  <div class="form-group">
    <div class="form-inline">
      <% if @contact.new_contract? || @contact.apartment_owner? %>
        ...
        <%= f.input :new_prefecture_code, collection: JpPrefecture::Prefecture.all, value_method: :code %>
...
<%= simple_form_for :calling_history, url: admin_lpgas_contact_calling_histories_path(@contact), method: 'post', html: {class: 'js-calling-history-add'} do |f| %>
  <div class="form-group">
    <div class="form-inline">
      <div class="form-group">
...
<div class="btn-group" role="group">
  <%= link_to new_admin_lpgas_contact_estimate_path(@contact), id:"check-value", class: 'btn btn-default' do %>
    <i class="fa fa-paper-plane"></i>
    ガス会社に見積もり依頼を送る
  <% end %>
  <%= link_to admin_lpgas_contact_estimates_path(@contact), class: 'btn btn-default' do %>
    <i class="fa fa-list"></i>
    見積もり依頼一覧
  <% end %>
  <%= link_to '提示画面を見る', lpgas_contact_path(@contact, token: @contact.token, pin: @contact.pin), target: "_blank", class: 'btn btn-default' %>
...
<% unless @contact.deleted_at %>
  <div style="margin-top: 2em">
    <%= link_to admin_lpgas_contact_path(@contact), data: {confirm: {text: 'この操作は取り消しができません。<br>また、見積り依頼は全てキャンセルされます。', title: '削除対象のユーザーの名前を正しく入力してください', confirmField: "削除対象者氏名", confirmValue: @contact.name}, method: 'delete'}, class: 'btn btn-danger' do %>
