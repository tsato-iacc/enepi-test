...
<table class="table table-condensed table-striped table-hover">
  ...
  <tbody>
    <tr>
      <td><?= MyView::link_to(@contact.id, admin_lpgas_contact_path(@contact) ?></td>
      <td><?= @contact.name ?></td>
...
<? else ?>
  <table class="table table-striped table-hover">
    ...
    <tbody>
      <? @sent_estimates.each { |estimate| ?>
        <?= fields_for 'estimates[]', estimate { |f| ?>
          <label>
            <tr>
              <td><?= MyView::link_to(estimate.uuid, admin_lpgas_estimate_path(estimate.uuid)  ?></td>
              <td>
                <? if estimate.expired? ?>
              ...
              <td><?= MyView::link_to(estimate.company.full_name, admin_lpgas_company_path(estimate.company_id) ?></td>
              ...
              <td>
                ...
                <? unless Rails.env.production? ?>
                  <? if estimate.expired_at ?>
                    <? if estimate.expired? ?>
                      <?= MyView::link_to("期限内にする(テスト用)", admin_lpgas_estimate_unexpire_path(estimate), ["method" => 'post', ["class" => 'btn btn-danger btn-xs' ?>
                    <? else ?>
                      <?= MyView::link_to("期限切れにする(テスト用)", admin_lpgas_estimate_expire_path(estimate), ["method" => 'post', ["class" => 'btn btn-danger btn-xs' ?>
                    <? } ?>
                  <? } ?>
                <? } ?>
              </td>
...
<? else ?>
  ...
  <? } ?>
  <?= MyView::form_tag admin_lpgas_contact_estimates_path(@contact) { |f| ?>
    <table class="table table-striped table-hover">
      <thead>
        <tr>
          <th>会社名</th>
      ...
      <tbody>
        ...
        <? else ?>
          <? @estimates.each.with_index { |estimate, idx| ?>
            <?= fields_for 'estimates', estimate, index: idx { |f| ?>
              <label>
                <tr>
                  <td>
