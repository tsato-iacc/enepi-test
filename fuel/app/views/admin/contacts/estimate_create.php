    ...
    <tbody>
      <?php foreach($contact->estimates as $estimate): ?>
        ...
        <tr>
          ...
          <td><a href="<?= \Uri::create('admin/partner_companies/:id/edit', ['id' => $estimate->company->id]); ?>"><?= $estimate->company->getCompanyName(); ?><?= $estimate->company->partner_company->company_name != $estimate->company->getCompanyName() ? '('.$estimate->company->partner_company->company_name.')' : ''; ?></a></td>
