...
abstract class Eauth_Group_Driver extends \Eauth_Driver
{
	/**
	 * @var	Auth_Driver
	 */
	...
	public static function forge(array $config = array())
	{
		...
		$class = \Inflector::get_namespace($config['driver']).'Eauth_Group_'.\Str::ucwords(\Inflector::denamespace($config['driver']));
	...
	public function has_access($condition, $driver, $group = null)
	{
		...
		if (is_array($group))
		{
			...
			return \Eauth::acl($driver)->has_access($condition, $group);
		}

		// When no group was given check all logged in users
		foreach (\Eauth::verified() as $v)
		{
