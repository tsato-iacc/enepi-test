...
abstract class Eauth_Login_Driver extends \Eauth_Driver
{
	/**
	 * @var  Eauth_Driver	default instance
	 */
	...
	public static function forge(array $config = array())
	{
		...
		$class = \Inflector::get_namespace($config['driver']).'Eauth_Login_'.\Str::ucwords(\Inflector::denamespace($config['driver']));
	...
	final public function check()
	{
		...
		{
			\Eauth::_unregister_verified($this);
	...
	public function member($group, $driver = null, $user = null)
	{
		...
		return \Eauth::group($driver)->member($group, $user);
	}
	...
	public function has_access($condition, $driver = null, $entity = null)
	{
		...
		return \Eauth::acl($driver)->has_access($condition, $entity);
	}
	...
	{
		return $encrypted_password = sha1("--{$salt}--{$password}--");
	}
	...
	public function groups($driver = null)
	{
		...
		if ($driver === null)
		{
			foreach (\Eauth::group(true) as $group)
			{
		...
		{
			$result = \Eauth::group($driver)->groups();
		}
