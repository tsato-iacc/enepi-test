...
class EauthException extends \FuelException {}
...
{
	/**
	 * @var  Eauth_Login_Driver	default instance
	 */
	...
	public static function _init()
	{
		...
		foreach((array) \Config::get('eauth.driver', array()) as $driver => $config)
		{
	 ...
	 * @throws  EauthException  on driver load failure
	 */
	public static function forge($custom = array())
	{
		...
		$config = \Config::get('eauth.'.$custom['driver'].'_config', array());
		$config = array_merge($config, $custom);

		// Driver must be set
		if (empty($config['driver']) || ! is_string($config['driver']))
		{
			throw new \EauthException('No auth driver given.');
		}

		// determine the driver to load
		$driver = \Eauth_Login_Driver::forge($config);
		...
		if (isset(static::$_instances[$id]))
		{
			...
			{
				throw new \EauthException('You can not instantiate two different login drivers using the same id "'.$id.'"');
			}
		}
		else
		{
		...
		if (count(static::$_instances) > 1)
		{
			...
			static::$_verify_multiple = \Config::get('eauth.verify_multiple_logins', false);
		}
	 ...
	 * @throws  EauthException  when $driver_id isn't valid or true
	 */
	...
	public static function check($specific = null)
	{
		...
		if ($specific !== null)
		{
			...
			{
				$i = $i instanceof Eauth_Login_Driver ? $i : static::instance($i);
				$key = $i->get_id();
	 ...
	 * @return  Array|Eauth_Login_Driver|false
	 */
	...
	public static function login($username_or_email = '', $password = '')
	{
		...
		{
			if ($i instanceof Eauth_Login_Driver)
			{
	 ...
	 *
	 * @param  Eauth_Login_Driver
	 */
	public static function _register_verified(Eauth_Login_Driver $driver)
	{
	 ...
	 * @param  Eauth_Login_Driver
	 */
	public static function _unregister_verified(Eauth_Login_Driver $driver)
	{
	...
	{
		$class = 'Eauth_'.\Str::ucwords($type).'_Driver';
